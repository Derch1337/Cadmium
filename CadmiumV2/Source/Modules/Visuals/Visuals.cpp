...
void DrawCapsule(const Vector2& p1, const Vector2& p2, float radius, ImU32 color) {
	...
	// Âû÷èñëÿåì íàïðàâëåíèå ìåæäó äâóìÿ òî÷êàìè
	Vector2 dir = p2 - p1;
	float length = std::sqrt(dir.x * dir.x + dir.y * dir.y);

	if (length > 0.0f) {
		dir.x /= length; // Íîðìàëèçóåì íàïðàâëåíèå
		dir.y /= length;
	}

	// Âû÷èñëÿåì ïåðïåíäèêóëÿðíîå íàïðàâëåíèå äëÿ øèðèíû êàïñóëû
	Vector2 perp(-dir.y * radius, dir.x * radius);

	// Âû÷èñëÿåì óãëîâûå òî÷êè ïðÿìîóãîëüíèêà (òåëà êàïñóëû)
	...
	// Ðèñóåì ïîëóñôåðû íà êîíöàõ
	draw_list->AddCircleFilled(Vector2ToImVec2(p1), radius, color, 32); // Ëåâàÿ ïîëóñôåðà
	draw_list->AddCircleFilled(Vector2ToImVec2(p2), radius, color, 32); // Ïðàâàÿ ïîëóñôåðà
}
